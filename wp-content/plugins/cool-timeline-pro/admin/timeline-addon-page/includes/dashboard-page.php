...
// Check if a pro version is already installed
if ( isset( $this->disable_plugins[ $plugin_slug ] ) ) {
	$pro_version = $this->disable_plugins[ $plugin_slug ];
	if ( isset( $pro_version['pro'] ) && file_exists( WP_PLUGIN_DIR . '/' . $pro_version['pro'] ) ) {
		$pro_already_installed = true;
		$classes              .= ' plugin-not-required';
	}
}

if ( isset( $plugin_slug ) && file_exists( WP_PLUGIN_DIR . '/' . $plugin_slug ) ) {
	...
	if ( isset( $plugin_url ) && $plugin_url != null ) {
...
if ( $pro_already_installed === true ) {
	...
	$button  = '<button class="button button-disabled" title="This plugin is no longer required as you already have ' . esc_html( $pro_ver['pro'] ) . '">Pro Installed</button>';
}
