	...
	class cool_plugins_timeline_addons {
		...
		public function displayPluginAdminDashboard() {
			...
			if ( ! empty( $plugins ) && count( $plugins ) > 0 ) {
				...
				foreach ( $plugins as $plugin ) {
					...
					$plugin_logo    = esc_url( $this->addon_plugins_logo( $plugin['slug'] ) );
				...
				if ( ! empty( $this->pro_plugins ) && count( $this->pro_plugins ) > 0 ) :
					...
					foreach ( $this->pro_plugins as $plugin ) {
						...
						$plugin_desc    = esc_html( $plugin['desc'] );
						$plugin_logo    = esc_url( $this->addon_plugins_logo( $plugin['slug'] ) );
						$plugin_pro_url = null !== $plugin['buyLink'] ? esc_url( $plugin['buyLink'] ) : null;
						$plugin_url     = null;
						$plugin_version = null;
						$plugin_slug    = esc_attr( $plugin['slug'] );
			...
			foreach ( $plugin_info as $plugin ) {
				if ( $plugin->tag == $tag ) {
					$this->pro_plugins[ $plugin->slug ] = array(
						...
						'buyLink'       => null !== $plugin->buy_url ? esc_url( $plugin->buy_url ) : null,
						'version'       => esc_html( $plugin->version ),
						'download_link' => null,
						'incompatible'  => esc_attr( $plugin->free_version ),
					);
		...
		public function addon_plugins_logo( $slug ) {
			...
			if ( isset( $logos_arr[ $slug ] ) ) {
				return esc_url( CTP_PLUGIN_URL . 'admin/timeline-addon-page/assets/images/' . $logos_arr[ $slug ] );
			} else {
				return esc_url( CTP_PLUGIN_URL . 'admin/timeline-addon-page/assets/images/default-logo.png' );
			}
		}
	}
