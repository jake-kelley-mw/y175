...
// Verify the AJAX request with a nonce for security.
if ( ! check_ajax_referer( 'ctl_preview', 'nonce', false ) ) {
	wp_die( __( 'Invalid nonce specified', 'text-domain' ), '', 400 );
}
...
if ( ! class_exists( 'CTL_Shortcode_Preivew' ) ) {
	...
	class CTL_Shortcode_Preivew {
		...
		public function ctl_shortcode_filter( $data ) {
			...
			$value        = esc_html( $value ); // Escape HTML entities in the attribute value.
			$value        = preg_replace( '/\s+/', '', $value ); // Remove white spaces from the attribute value.
			$value        = wp_unslash( $value ); // Removes slashes from a string or recursively removes slashes from strings within an array.
