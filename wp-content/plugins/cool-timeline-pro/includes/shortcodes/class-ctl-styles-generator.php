...
if ( ! class_exists( 'CTL_Styles_Generator' ) ) {
	...
	class CTL_Styles_Generator {
		...
		public static function ctl_global_style() {
			...
			if ( ! is_array( $ctl_options_arr ) ) {
				$ctl_options_arr = array();
			}
		 ...
		 *
		 * @param array $settings Timeline settings array.
		 */
		public static function styles_settings_vars( $settings ) {
			$background_type = isset( $settings['timeline_background'] ) ? sanitize_text_field( $settings['timeline_background'] ) : '0';
			$bg_color        = '';

			if ( '1' === $background_type ) {
				$bg_color = isset( $settings['timeline_bg_color'] ) ? sanitize_hex_color( $settings['timeline_bg_color'] ) : 'none';
			}

			$first_post_color    = isset( $settings['first_post'] ) ? sanitize_hex_color( $settings['first_post'] ) : '#02c5be';
			$second_post_color   = isset( $settings['second_post'] ) ? sanitize_hex_color( $settings['second_post'] ) : '#f12945';
			$content_bg_color    = isset( $settings['content_bg_color'] ) ? sanitize_hex_color( $settings['content_bg_color'] ) : '#f9f9f9';
			$content_color       = isset( $settings['content_color'] ) ? sanitize_hex_color( $settings['content_color'] ) : '#666666';
			$year_lbl_text_color = isset( $settings['year_label_color'] ) ? sanitize_hex_color( $settings['year_label_color'] ) : '#ffffff';
			$title_color         = isset( $settings['title_color'] ) ? sanitize_hex_color( $settings['title_color'] ) : '#fff';
			$year_lbl_bg_color   = isset( $settings['circle_border_color'] ) ? sanitize_hex_color( $settings['circle_border_color'] ) : '#025149';
			$main_title_color    = isset( $settings['main_title_color'] ) ? sanitize_hex_color( $settings['main_title_color'] ) : '#000';
			$custom_date_color   = isset( $settings['custom_date_color'] ) ? sanitize_text_field( $settings['custom_date_color'] ) : '';

			$ctl_date_color = '';
			if ( 'yes' === $custom_date_color ) {
				$ctl_date_color = isset( $settings['ctl_date_color'] ) ? sanitize_hex_color( $settings['ctl_date_color'] ) : '#fff';
			}

			$line_color         = isset( $settings['line_color'] ) ? sanitize_hex_color( $settings['line_color'] ) : '#025149';
			$line_filling_color = isset( $settings['line_filling_color'] ) ? sanitize_hex_color( $settings['line_filling_color'] ) : '#38aab7';
		 ...
		 *
		 * @param array $settings get typograph settings.
		 */
		public static function ctl_typo_output( $settings ) {
			$output        = '';
			$important     = '';
			$font_family   = ( ! empty( $settings['font-family'] ) ) ? sanitize_text_field( $settings['font-family'] ) : '';
			$backup_family = ( ! empty( $settings['backup-font-family'] ) ) ? ', ' . sanitize_text_field( $settings['backup-font-family'] ) : '';
			if ( $font_family ) {
			...
			foreach ( $properties as $property ) {
				if ( isset( $settings[ $property ] ) && $settings[ $property ] !== '' ) {
					$output .= $property . ':' . sanitize_text_field( $settings[ $property ] ) . $important . ';';
				}
			}
			...
			$unit = ( ! empty( $settings['unit'] ) ) ? sanitize_text_field( $settings['unit'] ) : 'px';

			$line_height_unit = ( ! empty( $settings['line_height_unit'] ) ) ? sanitize_text_field( $settings['line_height_unit'] ) : 'px';

			foreach ( $properties as $property ) {
				if ( isset( $settings[ $property ] ) && $settings[ $property ] !== '' ) {
					$unit    = ( $property === 'line-height' ) ? $line_height_unit : $unit;
					$output .= $property . ':' . sanitize_text_field( $settings[ $property ] ) . $unit . $important . ';';
				}
			}
			return $output;
		}
